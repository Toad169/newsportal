					...
					<div class="row">
						<div class="col-lg-6 col-xs-12">
							<div class="form-group">
								...
								<input type="text" name="user" id="user" class="form-control" placeholder="Masukkan Username" value="<?= $data[
          'username'
        ] ?>">
							</div>
						</div>
						...
						<div class="col-lg-12 col-xs-12">
							<div class="form-group">
								...
								<input type="file" name="file" id="file" class="form-control" placeholder="Masukkan File" value="<?= $data[
          'img'
        ] ?>">
							</div>
						</div>
						<div class="col-lg-12 col-xs-12">
							<div class="form-group">
								...
								<input type="pengguna" name="pengguna" id="pengguna" class="form-control" placeholder="Masukkan Nama Pengguna" value="<?= $data[
          'nama_pengguna'
        ] ?>">
							</div>
						</div>
  ...
  if (!empty($gambar)) {
    if (in_array($ekstensi, $ekstensi_boleh) === true) {
      if ($ukuran < 2000000) {
        ...
        $sql = mysqli_query(
          $con,
          "UPDATE tbl_users SET username='$user', password='$pass', nama_pengguna='$pengguna', img='$gambar' WHERE id_user='$id'",
        );
    ...
    if (!empty($pass)) {
      ...
      $sql = mysqli_query(
        $con,
        "UPDATE tbl_users SET username='$user', password='" .
          md5($pass) .
          "', nama_pengguna='$pengguna', img='$gambar' WHERE id_user='$id'",
      );
      echo "<script>alert('Data Berhasil Di ubah!')</script>";
      echo "<script>window.location.href='index.php?page=user'</script>";
    } else {
 ...
 <?php if (isset($_POST['tambah'])) {
   ...
   if (in_array($ekstensi, $ekstensi_boleh) === true) {
     if ($ukuran < 2000000) {
       ...
       $sql = mysqli_query(
         $con,
         "INSERT INTO tbl_users VALUES ('', '$user', '$pass', '$pengguna', '$gambar', '$lvluser')",
       );
